...
namespace nc
{
    namespace ida
    {
        ...
        namespace
        {
            ...
            {
                reinterpret_cast<
                    std::vector<std::pair<ByteAddr, QString>> *>(param)
                    ->emplace_back(ea, QLatin1String(name));
                return 1;
            }
        ...
        struct IdaFrontend::MenuItem : public ::action_handler_t
        {
            ...
            MenuItem(
                ...
                std::function<void()> handler) : path_(path),
                                                 name_(name),
                                                 handler_(handler),
        ...
        struct IdaFrontend::MenuItem
        {
            ...
            MenuItem(
                ...
                const QString &after,
                const QString &hotkey,
                std::function<void()> handler) : path_(path), name_(name), handler_(std::move(handler))
            {
                ::add_menu_item(
                    QString("%1/%2").arg(path).arg(after).toLocal8Bit().constData(),
                    name.toLocal8Bit().constData(),
        ...
        QWidget *IdaFrontend::createWidget(const QString &caption)
        {
            ...
            ::display_widget(widget, /*WOPN_MDI |*/ WOPN_DP_TAB /*| WOPN_DP_MENU*/);
